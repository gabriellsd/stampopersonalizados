...
export function FavoritesDrawer({ open, onClose }) {
  ...
  return (
    <>
      ...
      <div
        ref={drawerRef}
        className="fixed top-0 right-0 h-full w-full max-w-sm bg-white z-[60] shadow-2xl flex flex-col"
        aria-modal="true"
      ...
      >
        <div className="px-6 py-5 border-b flex justify-between items-center">
          ...
          <button type="button" onClick={onClose} aria-label="Fechar" className="p-2 text-gray-400 hover:text-black">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-4">
          {favorites.length === 0 ? (
            <p className="text-gray-500 text-sm text-center py-8">Nenhum favorito ainda. Clique no coração nos produtos.</p>
          ) : (
